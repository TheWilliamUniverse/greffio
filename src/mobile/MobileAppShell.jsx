...
import { Files, FolderKanban, Home, Search, UserRound } from 'lucide-react';
...
const isTabActive = (pathname, tabPath) => {
  ...
  if (tabPath === '/mobile/search') return pathname.startsWith('/mobile/search') || pathname.startsWith('/chat');
  if (tabPath === '/mobile/account') return pathname.startsWith('/mobile/account') || pathname.startsWith('/profil') || pathname.startsWith('/settings');
...
export const MobileAppShell = ({ children }) => {
  ...
  return (
    <div className="flex min-h-[100dvh] flex-col bg-[#f6f8fc]">
      ...
      <MobileTopBar />
      <main className="flex-1 overflow-y-auto pb-[calc(4.75rem+env(safe-area-inset-bottom))]">
        {children}
      </main>
