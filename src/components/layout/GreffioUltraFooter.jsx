  ...
  return (
    <footer
      ...
      className={cn(
        ...
        compact ? 'px-4 py-8' : 'px-4 py-10 sm:px-6 lg:px-8 lg:py-12',
        className,
      )}
    >
      <div className="mx-auto max-w-7xl">
        {!compact ? (
          <div className="flex flex-col gap-4 border-b border-white/10 pb-6 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-white/55">Greffio</p>
        ...
        {showIntro && !compact ? (
          <div className="mt-8 grid gap-8 border-b border-white/10 pb-8 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,1.9fr)]">
            ...
            >
              {columns.map((column) => (
                <FooterColumn key={column.id} title={column.title} links={column.links} compact={compact} />
              ))}
            </div>
          </div>
        ) : (
        ...
        <div className={cn('flex flex-col gap-4', compact ? 'mt-6' : 'mt-8 lg:flex-row lg:items-end lg:justify-between')}>
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs text-white/55">
            <span className="inline-flex items-center gap-1.5 rounded-md border border-white/15 px-2.5 py-1.5">
